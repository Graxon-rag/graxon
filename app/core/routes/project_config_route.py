from ..schemas.project_config_schema import ProjectConfigGetSchema, ProjectConfigUpdateSchema, ProjectConfigDetailGetSchema
...
@router.get("/{org_id}/{project_id}", response_model=ProjectConfigGetSchema)
async def get_project_config_by_project(org_id: str, project_id: uuid.UUID):
    try:
        handler = ProjectConfigHandler(org_id, project_id)
...
@router.get("/{org_id}/{project_id}/details", response_model=ProjectConfigDetailGetSchema)
async def get_with_details_by_project(org_id: str, project_id: uuid.UUID):
    try:
        handler = ProjectConfigHandler(org_id, project_id)
        result = await handler.get_with_details_by_project()
        if not result:
            logger.error({"message": "Failed to get project config", "result": result})
            return error_response("Failed to get project config", status.HTTP_404_NOT_FOUND)
        return success_response(data=result.model_dump(mode="json"))
