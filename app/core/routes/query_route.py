...
async def query(org_id: str, project_id: uuid.UUID, query: str = Query(..., description="Query text"), document_id: Optional[uuid.UUID] = Query(
    ...
    thinking: bool = Query(default=False, description="Thinking"),
    top_k: int = Query(
        default=10,
        ge=1,
        le=100,
    ...
    try:
        ...
        if thinking:
            # We return the StreamingResponse directly. 
            # Note: Do not 'await' handler.stream_query() here, as it returns a generator.
            return StreamingResponse(
                handler.stream_query(g_query), 
                media_type="text/event-stream"
            )

        # Handle Standard Request
        result = await handler.query(g_query)
